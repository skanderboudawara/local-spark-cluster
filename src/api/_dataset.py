...
class DataStorage:
    ...
    def __init__(self, extension: Optional[str] = None) -> None:
        ...
        if extension and extension not in set(["csv", "json", "parquet"]):
...
class Output(DataStorage):
    ...
    def append(self, df: DataFrame, partitionBy: int | str | list=None) -> None:
        """
    ...
    def write(self, df: DataFrame, partitionBy: int | str | list=None) -> None:
        """
        Writes the DataFrame to a local path in the specified format.
    ...
    def __prepare_dump(self, df: DataFrame, mode: str, partitionBy: int | str | list=None) -> None:
        """
        ...
        if mode not in set(["append", "overwrite"]):
